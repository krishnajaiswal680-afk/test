...
async def scrape_metars_to_dict(url):
    ...
    for script_or_style in soup(['script', 'style']):
        script_or_style.extract()
    text = soup.get_text(separator='\n')
    return parse_metars_from_text(text)


def append_to_json(filename, timestamp, stations):
    """Append structured data to a JSON file under timestamp key."""
    try:
...
async def main():
    ...
    V1 = await scrape_metars_to_dict(url1)  # OLBS
    V2 = await scrape_metars_to_dict(url2)  # AMSS

    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    not_in_amss, not_in_olbs, not_same, matches = {}, {}, {}, {}
    ...
    # Check reverse: stations in V2 but not in V1
    for station, metar2 in V2.items():
        if station not in V1:
            not_in_olbs[station] = [
                {"OLBS": None},
                {"AMSS": metar2}
    ...
    if not_in_amss:
        append_to_json("not_in_amss.json", timestamp, not_in_amss)
    if not_in_olbs:
        append_to_json("not_in_olbs.json", timestamp, not_in_olbs)
    if not_same:
        append_to_json("not_same.json", timestamp, not_same)
    if matches:  # optional, remove this block if you don’t want to store matches
        append_to_json("matches.json", timestamp, matches)
